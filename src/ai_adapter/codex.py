"""Codex CLI 어댑터(adapter) 구현.

비대화형(non-interactive) 모드: `codex exec "prompt"`
"""
...
class CodexAdapter(BaseAIAdapter):
    ...
    def _build_cli_args(
        ...
        """codex exec --full-auto --ephemeral 형식으로 인자 구성.

        --full-auto: 확인 프롬프트 없이 자동 실행
        --ephemeral: 세션 파일 저장 안 함 (일회성 호출)
        """
        base = [self._CLI_CMD, "exec", "--full-auto", "--ephemeral"]
        if use_stdin:
            return base + ["-"], prompt
    ...
    def is_available(self) -> bool:
        ...
        if not self._cli_exists():
            return False
        # codex exec는 응답 생성에 시간이 걸려 timeout 초과 가능
